...
async def generate_news_audio(request: NewsRequest):
    try:
        ...
        if request.source_type.lower() in ["news", "both"]:
            ...
            print(f"News results: {len(results.get('news', {}).get('news_analysis', {})) if results.get('news') else 0} topics")
        ...
        if request.source_type.lower() in ["reddit", "social", "both"]:
            ...
            print(f"Social results: {len(results.get('social', {}).get('social_analysis', {})) if results.get('social') else 0} topics")
